...
class LLMService {
    constructor() {
        ...
        this.provider = config.get('llmProvider') || 'openai';
        this.apiKey = config.get('llmApiKey') || '';
        this.model = config.get('llmModel') || 'gpt-4';
        this.localEndpoint = config.get('localLLMEndpoint') || 'http://localhost:8080/v1';
    ...
    initializeClient() {
        try {
            switch (this.provider) {
                case 'openai':
                    if (this.apiKey) {
                        this.openaiClient = new openai_1.default({ apiKey: this.apiKey });
                    }
                    break;
                case 'anthropic':
                    if (this.apiKey) {
                        // Make sure you have the latest Anthropic SDK
                        this.anthropicClient = new sdk_1.default({ apiKey: this.apiKey });
                    }
                    break;
    ...
    async configureLLM() {
        ...
        await vscode.commands.executeCommand('workbench.action.openSettings', '@ext:intelligent-debugger.llm');
    }
        ...
        try {
            switch (this.provider) {
                case 'openai':
                    ...
                    return ((_b = (_a = openaiResponse.choices[0]) === null || _a === void 0 ? void 0 : _a.message) === null || _b === void 0 ? void 0 : _b.content) || '';
                case 'anthropic':
     ...
     */
    async analyzeCodeWithLLM(code, filename, projectContext) {
        // Construct prompt for code analysis
        const systemPrompt = "You are an expert code analyzer specialized in debugging and identifying potential issues. " +
...
`;
    ...
    extractStructuredInfoFromText(text) {
        ...
        if (complexMatches) {
            for (const match of complexMatches) {
                ...
                if (complex)
                    result.codeUnderstanding.complexFunctions.push(complex);
            }
        }
        return result;
    }
     ...
     */
    async explainAnomaly(code, variableValues, anomalyDescription, executionContext) {
        const systemPrompt = "You are an expert code analyzer specialized in debugging and identifying potential issues. " +
            "Analyze the provided code deeply to identify complex areas, potential bugs, and data flow insights. " +
            "Focus on areas that might need debugging attention, such as complex control flows, error-prone patterns, " +
            "and potential edge cases. Be specific and concise in your analysis. " +
...
`;
     ...
     */
    async analyzeRootCause(anomalyData, relevantCode, executionHistory) {
        const systemPrompt = "You are an expert root cause analysis system, specialized in debugging software issues. " +
...
`;
     ...
     */
    async enhanceDebugPrompt(userPrompt, codeContext, currentState) {
        const systemPrompt = "You are an expert debugging assistant that helps enhance user debug prompts with deeper semantic understanding. " +
...
`;
     ...
     */
    async suggestFixes(rootCause, relevantCode, executionContext) {
        const systemPrompt = "You are an expert code repair system. Your task is to analyze identified root causes of bugs " +
 ...
 */
     ...
     */
    extractListItems(text, startMarker, endMarker) {
        if (!startMarker)
            return [];
